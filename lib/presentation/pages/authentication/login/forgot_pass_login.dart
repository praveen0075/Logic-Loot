            ...
            child: Padding(
              ...
              child: BlocConsumer<LoginBloc, LoginState>(
                listener: (context, state) {
                  if(state.isforgetOtpReqHasError){
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(state.message??"Failed to generate otp"),backgroundColor: Colors.red,));
                  }else if (state.isforgetOtpReqSucces){
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(state.message??"An otp will sent to your mobile number"),backgroundColor: Colors.green,));
                ...
                builder: (context, state) {
                  ...
                  return Column(
                    children: [
                      const Image(
                          image:
                              AssetImage("assets/images/forgot-pass_img.png"),
                          width: 300),
                      const Row(
                          ...
                          child: TextButton(
                              onPressed: () {
                                if (key.currentState!.validate()) {
                                  context.read<LoginBloc>().add(LoginEvent.requestToOTP(numbr: phoneController.text));
                                  // Navigator.push(
                                  //     context,
                                  //     MaterialPageRoute(
                                  //         builder: (ctx) =>
